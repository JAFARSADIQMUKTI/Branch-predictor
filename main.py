...
class Application(tk.Frame):
    ...
    def initial_screen(self):
        self.clear()
        tk.Button(self.inner, text="Compare simple n-bit saturating counters",
                  command=self.simple_n_bit_predictor).pack()
    ...
    def choose_bht_entries(self):
        ...
        def next():
            ...
            self.bht_entry = nBitPredictor(self.num_bits,
                                           0) if bht_type_entry.get() == "-bit saturating counter" else nBitAgreePredictor(
                self.num_bits, 0)
    ...
    def choose_additional_settings(self):
        ...
        if self.indexing_method == "PC":
            ...
            tk.Label(self.inner,
                     text="These bits of PC will be used to XOR corresponding PHT entry to index BHT").pack()
                ...
                for line in lines:
                    try:
                        ...
                        messagebox.showerror('Error',
                                             'File lines are not formatted properly. Each line must have PC and T/NT separated by whitespace.')
                        return

        tk.Label(self.inner, text="Alternatively, select a text file to simulate.").pack()
        tk.Label(self.inner,
                 text="Each line in the file must start with a PC value and T/NT separated by whitespace.").pack()
